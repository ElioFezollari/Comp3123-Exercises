...
--- In a controller we want to be able to detect which endpoints are being "hit" by the user. Express offers us the ability to do such thing, and extracting the Router specifically
allows us to make routers for different parts of our application making our code more modular.This way we can seperate different endpoints into different files and then combine them in the end in one file making the structure of our code cleaner and more readable and maintainable.In this lab exercise specifically using express.Router() does not give a significant advantage, however in other personal apps of mine I have built with express not using express.Router() and the modularity and seperation it provides would make the entire app a big disaster
...
--- Error handling is crucial to any app and Express applications are no different.The two ways I usually implement error handing are try catch blocks, and an error middleware function. Try catch are put inside specific methods in the controller, while the middleware is put in the very end to catch and log the error, aswell as send a response to the frontend that there was an error. The middleware function is also very useful when it comes to catching outside errors such as mongoose ones making it a pretty central point in error catching in bigger apps 
...
--- The line above checks if there is an environment variable named port and if so the server will listen to that specific port for request. If said port does not exist then the server will listen at port 8081. This is very important as in deployment we use environment variable for multiple reasons such as hiding sensitive information but also setting the port in which we will listen to for requests. When we deploy an app we want to have the ability to check and change the port at which we are listening to and environment variables allow us to do that without having to always config the code itself. 
